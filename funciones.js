...
function loadTareas(){
	...
	document.querySelector('.tporhacer').innerHTML="";
	document.querySelector('.tterminado').innerHTML="";
	...
	for(i=0; i<tareas.length;i++){
		...
		if(tareas[i].estatus == "pendiente"){
			document.querySelector('.tporhacer').innerHTML += elemento;
		}else if(tareas[i].estatus == "tterminado"){
			document.querySelector('.tterminado').innerHTML += elemento;
		}
	}//for
...
function cambiarEstado(id){
	tareas = JSON.parse(localStorage.getItem('tareas'));
	if(tareas[id].estatus == 'tterminado'){
